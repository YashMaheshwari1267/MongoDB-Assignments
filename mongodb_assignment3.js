...
2. db.addresses.aggregate({ $project: { resId: "$restaurant_id", name: "$name" } })
3. db.addresses.aggregate({ $project: { resId: "$restaurant_id", name: "$name", { $not: "$_id" }}})
4. db.addresses.aggregate({ $project: { "_id": 0, "restaurant_id": 1, "name": 1, "borough": 1, "address.zipcode": 1 } }).pretty()
5. db.addresses.aggregate([{ $group: { "borough": "$borough" } }, { $match: { "borough": "Bronx" } }, { $sort: { "borough": 1 } }, { $limit: 5 }]).pretty()
6. db.addresses.aggregate([{ $match: { "borough": "Bronx" } }])
7. db.addresses.aggregate([{ $match: { "borough": "Bronx" } }, { $limit: 5 }, { $skip: 5 }])
...
20. db.restaurants.find({ "grades.score": { $not: { $gt: 10 }}}, { "restaurant_id": 1,"name": 1, "borough": 1, "cuisine": 1 });
21. db.restaurants.find({ $or: [ { name: /^Wil/ },{"$and": [{ "cuisine": { $ne: "American " } }, { "cuisine": { $ne: "Chinees" } }]}]}, { "restaurant_id": 1, "name": 1, "borough": 1, "cuisine": 1 });
22. db.restaurants.find({ "grades.date": ISODate("2014-08-11T00:00:00Z"),"grades.grade": "A", "grades.score": 11 }, { "restaurant_id": 1, "name": 1, "grades": 1 });
...
32. db.restaurants.find({name: { $regex: "mon.*", $options: "i" }},{"name": 1,"borough": 1, "address.coord": 1, "cuisine": 1});
